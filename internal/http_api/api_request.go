...
func NewDeadlineTransport(connecTimeout time.Duration, requestTimeout time.Duration) *http.Transport {
	transport := &http.Transport{
		DialContext: (&net.Dialer{
			Timeout:   connecTimeout,
...
func NewClient(tlsConfig *tls.Config, connecTimeout time.Duration, requestTimeout time.Duration) *Client {
	transport := NewDeadlineTransport(connecTimeout, requestTimeout)
...
retry:
	...
	req.Header.Add("Accpet", "application/vnd.nsq; version=1.0")
