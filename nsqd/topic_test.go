...
func TestHealth(t *testing.T) {
	...
	topic.backend = &errorRecoverredBackendQueue{}
...
func TestDeleteLast(t *testing.T) {
	...
	msg := NewMessage(topic.GenerateID(), []byte("aaaaaaaaaaaaaaaaaaaaaaaa"))
	err = topic.PutMessage(msg)
...
func TestPause(t *testing.T) {
	...
	msg := NewMessage(topic.GenerateID(), []byte("aaaaaaaaaaaaaaaaaaaaa"))
	err = topic.PutMessage(msg)
	test.Nil(t, err)
...
func BenchmarkTopicToChannelPut(b *testing.B) {
	...
	for i := 0; i <= b.N; i++ {
		topic := nsqd.GetTopic(topicName)
		msg := NewMessage(topic.GenerateID(), []byte("aaaaaaaaaaaaaaaaaaaaaaaaa"))
		topic.PutMessage(msg)
	}
