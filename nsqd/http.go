...
func (s *httpServer) doPub(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	// for http, ContentLength is not a MUST requirement
	// what's the value of ContentLength if client did not pass this value ?
	...
	if int64(len(body)) == readMax {
		return nil, http_api.Err{Code: 500, Text: "MSG_TOO_BIG"}
	}
...
func (s *httpServer) doMPUB(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	if req.ContentLength > s.ctx.nsqd.getOpts().MaxBodySize {
		return nil, http_api.Err{Code: 403, Text: "BODY_TOO_BIG"}
	}
	...
	if vals, ok := reqParams["binary"]; ok {
		if binaryMode, ok = boolParams[vals[0]]; !ok {
			binaryMode = true
			s.ctx.nsqd.logf(lg.WARN, "deprecated value '%s' used for /mpub binary param", vals[0])
		}
	}
	...
	if binaryMode {
		...
		if err != nil {
			return nil, http_api.Err{Code: 403, Text: err.(*protocol.FatalClientErr).Code[2:]}
		}
	} else {
		...
		for !exit {
			...
			if err != nil {
				if err != io.EOF {
					// why don't need to log the reason of err but just return a 500
					return nil, http_api.Err{Code: 500, Text: "INTERNAL_ERROR"}
				}
				exit = true
			}
			...
			// sliently discard 0 length messages
	...
	err = topic.PutMessages(msgs)
	if err != nil {
		// the only error should be exiting
		return nil, http_api.Err{Code: 503, Text: "EXITING"}
	}
...
func (s *httpServer) printStats(stats []TopicStats, producerStats []ClientStats,
	ms memStats, health string, startTime time.Time, uptime time.Duration) []byte {
