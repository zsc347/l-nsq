...
func (s *FullJitterStrategy) Calculate(attempt int) time.Duration {
	...
	s.rngOnce.Do(func() {
		// why need this check when alrady wrapped by once ?
		if s.rng != nil {
			return
		}
...
type Config struct {
	...
	// To set TLS config, use following options
	//
	// tls_v1 - Bool enable TLS negotiation
	// tls_root_ca_file - String path to file containing root CA
	// tls_insecure_skip_verify - Bool indicates wheter this client should verify server cetificates
	...
	// tls_min_version - String indicating the minimum version of tls acceptable
	...
	//
	// WARNING: configuing clients with extremely low
	// (< 25ms) output_buffer_timeout has a significant effect
	// on nsqd CPU useage (particularyly with > 50 clients connected).
	OutputBufferTimeout time.Duration `opt:"output_buffer_timeout" default:"250ms"`

	// Maximum number of messages to allow in flight (concurrency knob)
	MaxInFlight time.Duration `opt:"msg_in_flight" min:"0" default:"1"`
...
func (h *structTagsConfig) SetDefaults(c *Config) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		filed := typ.Field(i)
		opt := filed.Tag.Get("opt")
		defaultVal := filed.Tag.Get("default")
		if defaultVal == "" || opt == "" {
			continue
		}
...
func (h *structTagsConfig) Validate(c *Config) error {
	...
	if c.HeartbeatInterval > c.ReadTimeout {
		return fmt.Errorf("HeartbeatInterval %v must be less than ReadTimeout %v",
...
// Set takes an options as a string and a value as an interface and
...
// 1000 (an interger interpreted as milliseconds)
...
// Validate checks all values are within specified min/max ranges
