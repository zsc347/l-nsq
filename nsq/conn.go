...
// MaxRDY return the nsqd negotiated maximum
...
func (c *Conn) identify() (*IdentifyResponse, error) {
	...
	if resp.Deflate {
		c.log(LogLevelInfo, "upgrade to Deflate")
...
func (c *Conn) auth(secret string) error {
	...
	c.log(LogLevelInfo, "Auth accepted. Identify: %q %s Permissions: %d",
...
func (c *Conn) readLoop() {
	...
	for {
		...
		case FrameTypeError:
			c.log(LogLevelError, "protoval error - %s", data)
...
func (c *Conn) close() {
	...
	// 3. set c.closeFlag
	// 4. readLoop() exits
	//     a. if message-in-flight > 0 delay close()
	//     b. else call close() immediately
	// 6a. lanch cleanup() gotoutine (we're racing with intraprocess
	//     routed messages, see comments below)
	//     a. wait on c.drainReady
	//     b. loop and receive on c.msgResponseChan chan util messages-in-flight == 0
	// 6b. launch waitForCleanup() goroutine
	//     b. wait on waitgroup (covers readLoop() and writeLoop() and cleanup goroutine)
...
func (c *Conn) cleanup() {
	<-c.drainReady // wait for write exit
	...
	for {
		...
		if msgsInFlight > 0 {
			if time.Now().Sub(lastWarning) > time.Second {
				c.log(LogLevelWarning, "draining ... waiting for %d messages in flight",
					msgsInFlight)
				lastWarning = time.Now()
			}
		}

		// until the readLoop has exited we cannot be sure that there still
		//  won't be a race
...
func (c *Conn) onMessageRequeue(m *Message, delay time.Duration, backoff bool) {
	if delay == -1 {
		...
		// bound the requeueDealy to configured max
		if delay > c.config.MaxRequeueDelay {
			delay = c.config.MaxRequeueDelay
		}
	}
	c.msgResponseChan <- &msgResponse{msg: m, cmd: Requeue(m.ID, delay), success: false}
}
